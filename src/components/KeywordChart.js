      ...
      {keywords.length > 0 ? (
        <ul>
          {keywords.map((keyword, index) => (
            <li key={index} className="flex items-start mb-4 text-md justify-start">
              <div className={`flex justify-start items-start gap-2 ${index < 3 ? 'font-700' : 'font-400 text-gray-700'}`}>
                <span className='w-[36px] flex justify-end'>{index + 1}위</span>
                <span className='w-[120px] break-all flex'>{keyword.name}</span>
              </div>
              <div className="flex w-full mt-2 bg-primary-50 ml-2 mr-2 rounded-full">
                <div
