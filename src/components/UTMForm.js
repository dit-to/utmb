...
const UTMForm = ({ onFocus }) => {
  ...
  return (
    <div>
      <div className="space-y-6">
        <div className='space-y-2'>
          ...
          <input
            ...
            required
            className="border border-gray-300 p-2 w-full rounded-md focus:shadow-[0_0_20px_0_rgba(97,95,225,0.3)] focus:outline-[1px] focus:outline-primary-500"
          />
        </div>
      ...
      {/* 기타 요소 */}
      <div className={`mt-4 p-4 rounded-lg ${isButtonEnabled ? 'bg-primary-100 outline outline-primary-500 outline-2' : 'bg-gray-200'}`}>
        <p className={`text-lg font-600 mb-2 ${isButtonEnabled ? '' : 'text-red-500'}`}>{isButtonEnabled ? '링크가 완성되었어요!' : '필수항목을 모두 입력해주세요'}</p>
        <input
          type="text"
          readOnly
          value={generatedLink}
          className="w-full p-2 border rounded-md focus:outline-none text-gray-700 bg-gray-100"
        />
        <button
          onClick={handleCopyLink}
          disabled={!isButtonEnabled}
          className={`mt-4 py-2 px-4 rounded-full h-[48px] w-full ${isButtonEnabled ? 'bg-primary-500 text-white font-600' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}
        >
          링크 복사하기
        </button>
      </div>
